...
export default function Hyperspeed({ effectOptions = DEFAULT_OPTS }) {
  ...
  useEffect(() => {
    ...
    const distortions = {
      ...
      mountainDistortion: {
        ...
        `,
        getJS: (progress, time) => {
          let m = 0.02, uFreq = mountainUniforms.uFreq.value, uAmp = mountainUniforms.uAmp.value;
          let d = new THREE.Vector3(
            Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.cos(m * Math.PI * uFreq.x + time) * uAmp.x,
            nsin(progress * Math.PI * uFreq.y + time) * uAmp.y - nsin(m * Math.PI * uFreq.y + time) * uAmp.y,
            nsin(progress * Math.PI * uFreq.z + time) * uAmp.z - nsin(m * Math.PI * uFreq.z + time) * uAmp.z,
          );
      ...
      xyDistortion: {
        ...
        `,
        getJS: (progress, time) => {
          let m = 0.02, uFreq = xyUniforms.uFreq.value, uAmp = xyUniforms.uAmp.value;
          let d = new THREE.Vector3(
            Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.cos(m * Math.PI * uFreq.x + time) * uAmp.x,
            Math.sin(progress * Math.PI * uFreq.y + time + Math.PI / 2) * uAmp.y - Math.sin(m * Math.PI * uFreq.y + time + Math.PI / 2) * uAmp.y,
            0,
          );
    ...
    const carLightsFragment = `
      #define USE_FOG;${THREE.ShaderChunk["fog_pars_fragment"]}
    ...
    const carLightsVertex = `
      #define USE_FOG;${THREE.ShaderChunk["fog_pars_vertex"]}
    ...
    const roadMarkings_fragment = `
      uv.y=mod(uv.y+uTime*0.05,1.);float laneWidth=1./uLanes;float blw=laneWidth*uBrokenLinesWidthPercentage;float les=1.-uBrokenLinesLengthPercentage;
      float brokenLines=step(1.-blw,fract(uv.x*2.))*step(les,fract(uv.y*10.));
      float sideLines=step(1.-blw,fract((uv.x-laneWidth*(uLanes-1.))*2.))+step(blw,uv.x);
      ...
      createPlane(side, width, isRoad) {
        ...
        if (isRoad) {
          uniforms = Object.assign(uniforms, {
            uLanes: { value: options.lanesPerRoad }, uBrokenLinesColor: { value: new THREE.Color(options.colors.brokenLines) }, uShoulderLinesColor: { value: new THREE.Color(options.colors.shoulderLines) },
            uShoulderLinesWidthPercentage: { value: options.shoulderLinesWidthPercentage }, uBrokenLinesLengthPercentage: { value: options.brokenLinesLengthPercentage }, uBrokenLinesWidthPercentage: { value: options.brokenLinesWidthPercentage },
          });
        }
        const material = new THREE.ShaderMaterial({
    ...
    class App {
      ...
      loadAssets() {
        return new Promise((resolve) => {
          ...
          si.addEventListener("load", function () { manager.itemEnd("smaa-search"); });
          ai.addEventListener("load", function () { manager.itemEnd("smaa-area"); });
      ...
      dispose() {
        this.disposed = true;
        if (this.scene) { this.scene.traverse((obj) => { if (!obj.isMesh) return; if (obj.geometry) obj.geometry.dispose(); if (obj.material) { if (Array.isArray(obj.material)) obj.material.forEach((m) => m.dispose()); else obj.material.dispose(); } }); this.scene.clear(); }
      ...
      setSize(w, h, updateStyles) { if (w <= 0 || h <= 0) { this.hasValidSize = false; return; } this.composer.setSize(w, h, updateStyles); this.hasValidSize = true; }
      tick() {
    ...
    const options = { ...DEFAULT_OPTS, ...effectOptions, colors: { ...DEFAULT_OPTS.colors, ...(effectOptions.colors || {}) } };
